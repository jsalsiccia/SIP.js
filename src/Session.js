  ...
  accept: function(options) {
    ...
    //mediaStream = options.mediaStream || null,
      ...
      descriptionCreationSucceeded = function(description) {
        ...
        if (ua.configuration.sessionTimer === SIP.C.supported.SUPPORTED) {
          // http://tools.ietf.org/html/rfc4028#section-9
          if (request.hasHeader('Session-Expires')) {
            var requestSessionExpires = request.parseHeader('Session-Expires');
            if (requestSessionExpires) {
              var responseSessionExpires = 'Session-Expires: ' + requestSessionExpires.deltaSeconds;
              if (requestSessionExpires.refresher) {
                responseSessionExpires += ';refresher=' + requestSessionExpires.refresher;
              } else {
                responseSessionExpires += ';refresher=' + ua.configuration.sessionTimerUasRefresherChoice;
              }
              extraHeaders.push(responseSessionExpires);
            } else {
              self.logger.error('failed to parse Session-Expires header');
            }
          } else if (ua.configuration.sessionTimerInitialRequest) {
            var supportedOptions = request.parseHeader('Supported') || [];
            if (supportedOptions.indexOf('timer') >= 0) {
              if (request.hasHeader('Min-SE')) {
                var minSe = request.parseHeader('Min-SE');
                if (minSe) {
                  if (minSe.deltaSeconds > ua.configuration.sessionTimerInterval) {
                    extraHeaders.push('Session-Expires: ' + minSe.deltaSeconds + ';refresher=' + ua.configuration.sessionTimerUasRefresherChoice);
                  } else {
                    extraHeaders.push('Session-Expires: ' + ua.configuration.sessionTimerInterval + ';refresher=' + ua.configuration.sessionTimerUasRefresherChoice);
                  }
                } else {
                  self.logger.error('failed to parse Min-SE header');
                  extraHeaders.push('Session-Expires: ' + ua.configuration.sessionTimerInterval + ';refresher=' + ua.configuration.sessionTimerUasRefresherChoice;)
                }
              } else {
                extraHeaders.push('Session-Expires: ' + ua.configuration.sessionTimerInterval + ';refresher=' + ua.configuration.sessionTimerUasRefresherChoice;)
        ...
        if (uasSupport && uacSupport && (
            request.hasHeader('Session-Expires') || ua.configuration.sessionTimerUASRefresher !== SIP.UA.C.REFRESHER_OMIT)) {
          var interval = sessionExpires.deltaSeconds || SIP.UA.C.SESSION_EXPIRES_DEFAULT;
          if (minSE.deltaSeconds && minSE.deltaSeconds > interval) {
            interval = minSE.deltaSeconds;
          }
          var refresher = sessionExpires.refresher;
          if (!refresher) {
            if (ua.configuration.sessionTimerUASRefresher !== SIP.UA.C.REFRESHER_OMIT) {
              refresher = ua.configuration.sessionTimerUASRefresher;
            } else {
              refresher = SIP.C.REFRESHER_UAS;
            }
          }
          extraHeaders.push('Session-Expires: ' + interval + ';refresher=' + refresher);
          if (refresher === SIP.C.REFRESHER_UAC || supportedOptions.indexOf('timer') >= 0) {
            extraHeaders.push('Require: timer');
          }
          this.setSessionTimer(interval, refresher, minSE.deltaSeconds);
        }
